...
class pfSenseUser(object):
    ...
    def _validate_password(self, user):
        if re.match(r'\$2b\$', user['password']):
            user['sha512'] = user['password']
        else:
            self.module.fail_json(msg='Password (%s) does not appear to be a sha512 hash' % user['password'])
        del user['password']
    ...
    def add(self, user):
        ...
        if user_elt is None:
            ...
            self.change_descr = 'ansible pfsensible.core.user added %s' % (user['name'])
        else:
            if 'password' in user:
                self._validate_password(user)
            ...
            self.change_descr = 'ansible pfsensible.core.user updated "%s"' % (user['name'])
    ...
    def remove(self, user):
        ...
        if changed and not self.module.check_mode:
            ...
            self.pfsense.write_config(descr='ansible pfsensible.core.user removed "%s"' % (user['name']))
        self.module.exit_json(changed=changed, diff=self.diff, stdout=stdout, stderr=stderr)
